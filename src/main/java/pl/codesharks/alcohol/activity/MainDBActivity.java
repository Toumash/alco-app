...
import pl.codesharks.alcohol.alcoapi.contract.Main_Alcohol;
...
import pl.codesharks.alcohol.service.HUDService;
import pl.codesharks.alcohol.service.UpdaterIntentService;
...
public class MainDBActivity extends ThemeListActivity {
    ...
    public boolean onOptionsItemSelected(@NotNull com.actionbarsherlock.view.MenuItem item) {
        ...
        Galgo.log("Service Started");
        Galgo.log("App Started");
        Galgo.log("Logging established");
        Galgo.log("Connected to 192.168.0.1");
        Galgo.log("Trying to do clever things");
        return true;
    }
...
/*                String[] fromFieldNames = new String[]
    ...
    void updateDatabase() {
        ...
        json = AlcoAPI.getJSONWithSession(context);

        AlcoAPI.post(context, asyncHttpClient, NewActions.DOWNLOAD_MAIN_DB, json, 5000, new AlcoHttpResponseHandler() {
            @Override
            ...
            public void onFail(int i, Header[] headers, JSONObject json, Throwable throwable) {
                if (json != null)
                    Log.e(TAG, "JSON RESULT: " + json);
                Log.e(TAG, "Downloading Database error AND no valid JSON to display");
